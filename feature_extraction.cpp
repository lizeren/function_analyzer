...
//                 functionJson["total_local_variables"] = LVC.typeCount.size();
...
class FunctionAnalyzer : public MatchFinder::MatchCallback {
public:
    std::vector<ordered_json> functionData;  // Store JSON objects in an ordered fashion

    void run(const MatchFinder::MatchResult &Result) override {
        if (const FunctionDecl *FD = Result.Nodes.getNodeAs<FunctionDecl>("functionDecl")) {
            if (!FD->getLocation().isInvalid() && Result.SourceManager->isInMainFile(FD->getLocation())) {
                ordered_json functionJson;
                functionJson["function_name"] = FD->getNameAsString();
                functionJson["number_of_parameters"] = FD->getNumParams();
                ...
                // Assume LocalVariableCounter LVC is filled properly here
                functionJson["total_local_variable_stack_size_bytes"] = 65; // Example static data
                functionJson["total_local_variables"] = 11; // Example static data
    ...
    ~FunctionAnalyzer() {
        ...
        jsonFile << outputJson.dump(4);  // Write with an indentation of 4 spaces
        jsonFile.close();
    }
};
